...
# Choose how many sheets you want to automate
Automate_num_report = 10
...
###############################################################################################################
    ...
    #For Coding purposes. i am stupid
    global s1
    s1 = exhibit1  
...
def ex2(row_id):
    ...
    #If the title is too big, then autosize text to perfect fit
    if len(str(row_id)) > 28:
...
################################################################################  
def delete_slide(obj):

    id_dict = { obj.id: [i, obj.rId] for i,obj in enumerate(prs.slides._sldIdLst) }
    slide_id = obj.slide_id
    prs.part.drop_rel(id_dict[slide_id][1])
    del prs.slides._sldIdLst[id_dict[slide_id][0]]

    #slide_id = drop_rel(id_dict[slide_id][1])
    #del prs.slides._sldIdLst[id_dist[slide_id][0]]


def saveppt(file_name):

    #Saving the new powerpoint file
    file_name = str("final_reports/") + str(file_name + '.pptx')
    prs.save(file_name)
    delete_slide(s1)
    delete_slide(s2)
...
def main():
    ...
    global Automate_num_report
    # Automation Enabled
    #Getting number of companies, excluding the last 5 AVG values
    num_rows = df.shape[0] - 5
    
    if Automate_num_report > num_rows:
        Automate_num_report = num_rows

    for x in range(Automate_num_report):
        
        
        ex1(df.index.values[x],[14,15,16])
        ex2(df.index.values[x])
        saveppt(df.index.values[x])
